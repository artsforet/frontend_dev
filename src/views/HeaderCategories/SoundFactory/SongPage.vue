...
export default {
  ...
  computed: {
    ...
    userId() {
      return this.getUserId; // this.getUserId는 mapGetters를 통해 가져온 getter
    },
    currentItems() {
  ...
  methods: {
    ...
    async loadAudio() {
      try {
        for (const record of this.musicRecords) {
          ...
          const container = this.$refs.waveform.children[this.wavesurfers.length]
          if (container) {
            const wavesurfer = WaveSurfer.create({
              container: container.querySelector('.waveform-inner'),
              waveColor: 'gray',
    ...
    async playPauseAudio(index) {
      if (this.playingIndex === index) {
        ...
        this.playingIndex = -1;
        // EventBus.$emit('play-event', { index, status: 'paused' });
      } else {
        ...
        const song = this.currentItems[index];
        console.log('song' + song)
        // console.log("[SONG]"+ song);
        // alert(Object.entries(Object.entries(song)));

        const refreshToken = Cookies.get('refresh_token'); // 쿠키에서 리프레쉬 토큰을 가져옴
        try {
          // const get_user = await axios.get('/auth/profile');
          // const id = get_user.data.userId;
          const id = this.userId
          const response = await axios.post('/playlist/add-music', {
            userId: id,
            playlistId: 0,
            musicId: 41,
          }, {
            ...
            withCredentials: true // 쿠키를 포함하여 요청을 보낼 때 사용
          })
          if (response.status === 200) {
            this.$store.commit('ADD_SONG_TO_PLAYLIST', this.song); // 스토어 업데이트
            this.song.title = '';
            this.song.artist = '';
          }
        } catch (error) {
      ...
      //   const song = this.currentItems[index]
      //   // console.log("[SONG]"+ song);
      //   const result = Object.entries(song)
      //   // alert(Object.entries(Object.entries(song)))
      //   const refreshToken = Cookies.get('refresh_token'); // 쿠키에서 리프레쉬 토큰을 가져옴
      //
      //   try {
      //     await axios.post('/playlist/update', {
      //         name: song.Key,
      //         songId: 10,
      //         isPlaying: true
      //       }, {
